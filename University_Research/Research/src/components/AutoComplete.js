...
class AutoComplete extends React.Component{
  ...
  keyInput(){
           ...
           {
             const cmpstrno=this.props.List[i].fields.scholar.substring(0,size);
	...
	keyReturn(event){
        ...
        {  
          ...
          {	
           this.setState({typed:this.state.selected.fields.scholar,shortList:[]}); 
  ...
  keyDown(event){
	...
	render()
	{
        ...
        const renderList=this.state.shortList.map((short,index)=>{
        	return(
        		<li key={index} className={(short===this.state.selected)?"selected":"unselected"} id={short.fields.scholar} onClick={this.handleClick}>{short.fields.scholar}-{short.fields.name}</li>
        		);
        });
		return (
